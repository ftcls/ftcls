...
function readAsText(){
    ...
    reader.onload = function(e){
        ...
        resulttxt.innerText = e.target.result;
        var text = [];
        resulttxt.innerText.split("\n").forEach(function(s,i){text.push(s);});
        //alert(text.length);
        resulttxt.innerText='';
        var content = [];
        ...
        $('.nav-start button').html('Start');
        $('#upload').fadeOut(500);
        $("ul.question-item li:gt(4)").hide();
...
function downloadAsText(){
    var last_str = $("#finalresults .questionlist:last-child").html();
    var str_last = $("#finalresults .questionlist:last").remove().text();
    var str = $("#finalresults .questionlist").append("\n").text();
    var text = str+str_last;
    //var text = str.text();
    var blob = new Blob([text], {type: "text/plain;charset=utf-8"});
    saveAs(blob, "results.txt");
    $("#finalresults").append("<li class='questionlist'>"+last_str+"</li>");
    //alert(text);
}
...
(function($){
    $.fn.jquizzy = function(settings) {
        ...
        answers = [],
        introFob = '<div class="intro-container slide-container nav-start">Text Annotation Tool<br/><br/><span href="#" class="nav-start"><button type="button">Demo</button></span></div>	',
        exitFob = '<div class="results-container slide-container"><div class="download"><input type="button" value="Download" onclick="downloadAsText();"/></div><div class="result-keeper"></div></div>',
        contentFob = '',
        questionList = '',
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
            ...
            contentFob += '</ul><div class="notice">Please choose Yes or No！</div><div class="nav-container">';
        ...
        superContainer.find('.questionlist').click(function() {
            var num = $(this).index();
        ...
        superContainer.find('.final').click(function(){
            ...
            var sum_Label_1 = $("#slide-left-container .questionlist.label1").length;
            var sum_Label_2 = $("#slide-left-container .questionlist.label2").length;
            ...
            finalresults = '<ul id="finalresults" class="question-item">'+$('#slide-left-container').html()+'</ul><div class="page_btn clear"><span class="page_box"><a class="prev_page">Prev</a><span class="num"><span class="current_page">1</span><span style="padding:0 3px;">/</span><span class="total"></span></span><a class="next_page">Next</a></span></div>';
            resultSet = '<h2 class="qTitle">Your annotation results： <br/>' + resultCount + '</h2>' + finalresults + '<div class="jquizzy-clear"></div>';
            superContainer.find('.result-keeper').html(resultSet).show(500);
            setTimeout(function(){$('#left-item').html("");},800);
            //$('#left-item').html("");

            if($("#finalresults li").length>0){
                $("#finalresults li").show();
                $("#finalresults li:gt(4)").hide();//初始化，前面5条数据显示，其他的数据隐藏。
                var total_q=$("#finalresults li").index()+1;//总数据
                var current_page=5;//每页显示的数据
                var current_num=1;//当前页数
                ...
                $(".next_page").click(function(){
                    if(current_num==total_page){
                        ...
                        $(".current_page").text(++current_num);//点击下一页的时候当前页数的值就加1
                        $.each($('#finalresults li'),function(index,item){
                            var start = current_page* (current_num-1);//起始范围
                            var end = current_page * current_num;//结束范围
                            if(index >= start && index < end){//如果索引值是在start和end之间的元素就显示，否则就隐
                                $(this).show();
...
$(document).ready(function(){
    $("ul.question-item li:gt(4)").hide();//初始化，前面5条数据显示，其他的数据隐藏。
    var total_q=$("ul.question-item li").index()+1;//总数据
    var current_page=5;//每页显示的数据
    var current_num=1;//当前页数
    var total_page= Math.round(total_q/current_page);//总页数  
